...
test(`should call the tagging function`, () => {
  ...
  function tagIt(literalString, ...interpolatedParts) {
    ...
    const addToNoun = `super-cool ${interpolatedParts[0]}`
    const addToEmotion =`really ${interpolatedParts[1]}`
    

    return `${literalString[0]}${addToNoun}${literalString[1]}${addToEmotion}${literalString[2]}`
  }
})
